...
export async function sendNotificationEmail(toEmail, toName, fromName, message, listingTitle) {
  if (!EMAILJS.publicKey || EMAILJS.publicKey === "YOUR_PUBLIC_KEY") {
    console.warn('[Xenia] EmailJS not configured — skipping email');
    return;
  }
  if (!toEmail) {
    console.warn('[Xenia] No recipient email — skipping');
    return;
  }
  try {
    ...
    if (res.ok) {
      console.log('[Xenia] Email sent to', toEmail);
    } else {
      const errText = await res.text();
      console.error('[Xenia] Email failed:', res.status, errText);
    }
  } catch(e) {
    console.error('[Xenia] Email error:', e);
  }
}
