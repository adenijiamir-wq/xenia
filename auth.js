import {
  ...
  signOut, signInWithPopup, updateProfile as updateAuthProfile,
...
import { auth, rtdb, googleProvider } from './config.js';
import { state } from './state.js';
import { toast, showModal, closeModal, $ } from './helpers.js';
...
export function openAuth(initialMode = 'signin') {
  ...
  const render = () => {
    ...
    document.getElementById('authForm').addEventListener('submit', async e => {
      ...
      try {
        if (mode === 'signin') await doSignIn(fd.get('email'), fd.get('password'));
        else await doSignUp(fd.get('email'), fd.get('password'), fd.get('name'));
        closeModal();
      } catch (err) {
...
export function watchUnread() {
  ...
  const unsub = onValue(userConvsRef, async snap => {
    if (!snap.exists()) { updateBadge(0); return; }
    const convIds = Object.keys(snap.val());
    let total = 0;
    for (const cid of convIds) {
      const cs = await get(ref(rtdb, `conversations/${cid}/unread/${state.user.uid}`));
      if (cs.exists()) total += cs.val() || 0;
    }
    updateBadge(total);
  });
