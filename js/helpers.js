...
const SIZE_MAP  = { xs:'w-6 h-6 text-[10px]', sm:'w-8 h-8 text-xs', md:'w-10 h-10 text-sm', lg:'w-16 h-16 text-xl', xl:'w-24 h-24 text-3xl' };

export function avatarHtml(user, size = 'md') {
  const cls = SIZE_MAP[size] ?? SIZE_MAP.md;
  if (user?.photoURL) {
    return `<img src="${escapeHtml(user.photoURL)}" class="${cls} rounded-full object-cover" loading="lazy">`;
  }
  const name  = user?.displayName || user?.name || '?';
  const seed  = (user?.uid || name).split('').reduce((a, c) => a + c.charCodeAt(0), 0);
  const color = AV_COLORS[seed % AV_COLORS.length];
  return `<div class="${cls} ${color} rounded-full flex items-center justify-center text-white font-semibold">${initials(name)}</div>`;
}
...
export function showModal(html) {
  ...
  root.innerHTML = `
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-4 fade-in" id="modalBackdrop">
      <div class="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[92vh] overflow-y-auto slide-up">
        ${html}
      </div>
    </div>`;
  ...
  return `
    <a href="#listing/${l.id}" class="block group">
      <div class="relative aspect-square bg-ink-100 rounded-2xl overflow-hidden mb-2">
        ${photo
          ? `<img src="${escapeHtml(photo)}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300" loading="lazy">`
